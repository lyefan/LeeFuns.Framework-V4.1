...
namespace LeeFuns.Business.CommonModule.Oracle
{
    ...
    public class Base_UserBll : RepositoryFactory<Base_User>
    {
        ...
        public Base_User UserLogin(string Account, string Password, out string result)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            List<DbParameter> parameter = new List<DbParameter>();
            strSql.Append("SELECT * FROM Base_User WHERE 1=1 AND ROWNUM <= 1");
            strSql.Append(" AND UserAccount=:UserAccount");
            parameter.Add(DbFactory.CreateDbParameter(":UserAccount", Account));
            Base_User entity = Repository().FindEntityBySql(strSql.ToString(),parameter.ToArray());
